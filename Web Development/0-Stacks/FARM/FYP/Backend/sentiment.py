...
def getNews(brand: str, competitor: str, hashtag: str, p_id: int):
    ...
    competitors = session.query(newsCompetitor).filter(
        newsCompetitor.name == competitor).all()
    N_c_positive, N_c_negative, N_c_neutral = getSentiment(competitors)
    hashtags = session.query(newsHashtag).filter(
        newsHashtag.name == hashtag).all()
    N_h_positive, N_h_negative, N_h_neutral = getSentiment(hashtags)

    r_brands = session.query(redditBrands).filter(
        redditBrands.name == brand).all()
    R_b_positive, R_b_negative, R_b_neutral = getSentiment(r_brands)
    r_competitors = session.query(redditCompetitor).filter(
        redditCompetitor.name == competitor).all()
    R_c_positive, R_c_negative, R_c_neutral = getSentiment(r_competitors)
    r_hashtags = session.query(redditHashtag).filter(
        redditHashtag.name == hashtag).all()
    ...
    n_p_brand_sentiments = {"positive": N_b_positive,
                            "negative": N_b_negative, "neutral": N_b_neutral}
    n_p_competitor_sentiments = {
        "positive": N_c_positive, "negative": N_c_negative, "neutral": N_c_neutral}
    n_p_hashtag_sentiments = {"positive": N_h_positive,
                              "negative": N_h_negative, "neutral": N_h_neutral}

    r_p_brand_sentiments = {"positive": R_b_positive,
                            "negative": R_b_negative, "neutral": R_b_neutral}
    r_p_competitor_sentiments = {
        "positive": R_c_positive, "negative": R_c_negative, "neutral": R_c_neutral}
    r_p_hashtag_sentiments = {"positive": R_h_positive,
                              "negative": R_h_negative, "neutral": R_h_neutral}
    ...
    p_sentiments = {"positive": p_Positve,
                    "negative": p_negative, "neutral": p_neutral}
...
def updateProjectSentiment(brand: str, competitor: str, hashtag: str, p_id: int):
    ...
    p_sentiments = {"positive": p_Positve,
                    "negative": p_negative, "neutral": p_neutral}
    session.query(projectSentiments).filter(projectSentiments.project_id == p_id).update({
        projectSentiments.r_p_brand_sentiments:r_p_brand_sentiments,
        projectSentiments.r_p_competitor_sentiments:r_p_competitor_sentiments,
        projectSentiments.r_p_hashtag_sentiments:r_p_hashtag_sentiments,
        projectSentiments.n_p_brand_sentiments:n_p_brand_sentiments,
        projectSentiments.n_p_competitor_sentiments:n_p_competitor_sentiments,
        projectSentiments.n_p_hashtag_sentiments:n_p_hashtag_sentiments,
        projectSentiments.r_p_sentiments:r_p_sentiments,
        projectSentiments.n_p_sentiments:n_p_sentiments,
